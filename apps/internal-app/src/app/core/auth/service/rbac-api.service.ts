...
import { mapCurrentUserToRbacSnapshot } from '../mappers/user-snapshot.mapper';
import { UserPermissionResponseDto } from '../../api/api-client/models';

@Injectable({ providedIn: 'root' })
export class RbacApiService implements RbacDataSource<AppRbacUserAccessSnapshot> {
  constructor(
  ...
  fetchCurrentUserSnapshot(): Observable<AppRbacUserAccessSnapshot> {
    return this.fetchCurrentUser().pipe(map(mapCurrentUserToRbacSnapshot));
  }

  fetchCurrentUser(): Observable<CurrentUserResponseDto> {
    return this.authMe.meControllerGetMe().pipe(map((response) => response.responsePayload!));
  }

  fetchUserAccess(idpSub: string): Observable<RbacResponseDto> {
    return this.authUserRoles.userRolesControllerResolveAccess({ idpSub }).pipe(map((response) => response.responsePayload!));
  }
  ...
  listUserPermissions(idpSub: string, all: boolean = false): Observable<UserPermissionResponseDto[]> {
    return this.authUserRoles.userRolesControllerListUserPermissions({ idpSub, all: all ? 'true' : undefined })
      .pipe(map((response) => response.responsePayload ?? []));
  }
  ...
  grantPermission(idpSub: string, body: { permissionKey: string; note?: string }) {
    return this.authUserRoles.userRolesControllerGrantPermission({ idpSub, body }).pipe(map((response) => response.responsePayload!));
  }

  revokePermission(idpSub: string, grantId: string) {
    return this.authUserRoles.userRolesControllerRevokePermission({ idpSub, grantId }).pipe(
      map((response) => response.responsePayload!),
    );
  }
