...
import { isEmpty } from 'src/app/shared/utils/utilities.service';
...
export class NotificationModalComponent implements OnInit, OnDestroy {
  ...
  protected copyableRefIds: string[] = [];
  protected copyableRefsLabel = 'Ref IDs';
  protected copiedRefId?: string;
  ...
  constructor(
    ...
    this.details = data;
    this.copyableRefIds = data.copyableRefIds ?? [];
    this.copyableRefsLabel = data.copyableRefsLabel?.trim() || 'Ref IDs';
  ...
  protected copyRefId(refId: string): void {
    this.writeClipboard(refId).then(() => {
      this.copiedRefId = refId;
      if (this.copyResetTimer) {
        clearTimeout(this.copyResetTimer);
      }
      this.copyResetTimer = setTimeout(() => {
        this.copiedRefId = undefined;
      }, 1800);
    });
  }

  protected copyAllRefIds(): void {
    if (!this.copyableRefIds.length) {
      return;
    }

    this.writeClipboard(this.copyableRefIds.join(', ')).then(() => {
      this.snackBar.open(`${this.copyableRefsLabel} copied`, undefined, { duration: 1800 });
    });
  }
...
export interface NotificationData {
  ...
  copyableRefIds?: string[];
  copyableRefsLabel?: string;
