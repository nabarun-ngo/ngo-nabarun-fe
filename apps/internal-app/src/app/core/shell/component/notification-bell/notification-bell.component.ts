...
import {
  ...
  AppNotification,
  PagedNotifications,
} from 'src/app/core/shell/tokens/notification.token';
...
export class NotificationBellComponent implements OnInit, OnDestroy {
  ...
  private sheetLocked = false;
  private readonly mobileQuery = typeof window !== 'undefined'
    ? window.matchMedia('(max-width: 640px)')
    : null;

  constructor(
    @Inject(INotificationService) public notificationService: INotificationService,
  ...
  private acquireSheetLock(): void {
    if (this.sheetLocked || !this.mobileQuery?.matches) {
      return;
    }
    this.sheetLocked = true;
    setMobileSheetOpen(true);
  }

  private releaseSheetLock(): void {
    if (!this.sheetLocked) {
      return;
    }
