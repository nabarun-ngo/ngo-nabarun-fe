export const AppDialog = {
  logout_dialog: {
    title: 'Confirm Logout',
    description: 'Are you sure to logout from current session ?',
  },
  err_min_1_doc: {
    title: 'Error',
    description: 'Please upload at least one document.',
  },
  err_incorrect_role: {
    title: 'Error',
    description: 'There is some errors in role selection.',
  },
  err_no_change_made: {
    title: 'Error',
    description: 'No changes has been made.',
  },
  warn_delegated_request: {
    title: 'Warning',
    description:
      'You are raising a request on behalf of someone. Please ensure that person is aware.',
  },
  warn_confirm_withdraw: {
    title: 'Warning',
    description: 'Are you sure to withdraw this request?',
  },
  err_min_1_expense: {
    title: 'Error',
    description: 'Expense List must have atleast one expense.',
  },
  warning_confirm_revoke: {
    title: 'Warning',
    description:
      'Applications using this token will stop working. Are you sure to revoke this token?',
  },
  warn_confirm_finalize: {
    title: 'Warning',
    description:
      'No changes are allowed once expense is finalized. Are you sure to continue?',
  },
  warn_confirm_reject:{
    title: 'Warning',
    description:
      'Are you sure to reject this expense?',
  }
};
