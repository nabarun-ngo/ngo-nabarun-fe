...
import { DocumentDetail, DonationDetail, DonationStatus, PaymentMethod, UserDetail } from 'src/app/core/api/models';
import { DonationRefData, donationTab } from '../donation.const';
import { DonationService } from '../donation.service';
import { DonationList } from '../donation.model';
import { SharedDataService } from 'src/app/core/service/shared-data.service';
import { compareObjects, getNonNullValues } from 'src/app/core/service/utilities.service';
...
export class DonationAccordionComponent implements OnInit {
    ...
    if (result?.action == 'edit') {
      if (isConfirmed == true) {
        ...
        if (result.update?.valid) {
          let isFileNeeded= result.update?.donation.status == this.donationStatus.Paid && result.update?.donation.paymentMethod && result.update?.donation.paymentMethod != PaymentMethod.Cash;
          if (isFileNeeded && (!result?.upload || result?.upload?.length == 0)) {
            ...
            let update = compareObjects(result.update?.donation, result.donation) as DonationDetail;
            if(isFileNeeded){
  ...
  addDonation(isConfirmed: boolean) {
    if (isConfirmed) {
      ...
      if (this.createDonationData.validDonation && (this.tabName != 'guest_donation' || this.createDonationData.validGuest)) {
        ...
        this.donationService.createDonation(donation).subscribe(data => {
          ...
          this.createDonationData = { donation: {} ,eventSubject: new Subject<any>()};
  ...
  onDonationChange(donationData: { valid?: boolean | undefined; donation?: DonationDetail | undefined; }) {
  ...
  onProfileChange(memberData: { valid?: boolean | undefined; member?: UserDetail | undefined; }) {
